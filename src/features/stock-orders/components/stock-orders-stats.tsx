...
interface StockOrdersStatsProps {
  orders?: unknown[] // 保留 orders prop 以保持兼容性，但不再使用
}

export function StockOrdersStats({ orders: _orders }: StockOrdersStatsProps) {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch stock order statistics:', error)
        // 保持默认值
      } finally {
        setIsLoading(false)
      }
