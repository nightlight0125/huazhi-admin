...
const shippingMethodOptions = ['请选择', 'Standard Shipping', 'Express Shipping', 'TDPacket Electro'] as const
...
export const storeProducts: StoreProduct[] = Array.from({ length: 100 }, () => {
  return {
    ...
    hzPrice: faker.datatype.boolean({ probability: 0.3 }) ? parseFloat(faker.commerce.price({ min: 5, max: 80 })) : null,
    shippingFrom: faker.helpers.arrayElement(shippingFromOptions),
    shippingMethod: faker.datatype.boolean({ probability: 0.5 }) ? faker.helpers.arrayElement(shippingMethodOptions) : null,
